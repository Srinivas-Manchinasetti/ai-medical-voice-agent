...
void main() {
  ...
  float aspect = max(uResolution.x / uResolution.y, 1.0);
  float normX = uv.x / (aspect * 0.55);
  float e = 0.06 + uIntensity * 0.94;
  float env = pow(max(cos(clamp(normX, -1.0, 1.0) * PI * 0.5), 0.0), uTaper);
  ...
  if (uLightMode == 1) {
    // Saturated high-contrast mode for white/light backgrounds
    float gray = dot(col, vec3(0.299, 0.587, 0.114));
    vec3 saturatedCol = mix(vec3(gray), col, uSaturation);
    float maxC = max(max(saturatedCol.r, saturatedCol.g), saturatedCol.b);
    vec3 pureHue = maxC > 0.001 ? saturatedCol / maxC : vec3(0.0);
    
    // Core definition and soft luminous bloom
    float core = clamp(totalGlow * uGlow * 0.42, 0.0, 1.0);
    float alpha = clamp(core * uOpacity, 0.0, 0.95);
    
    // Rich saturated pigment tone
    vec3 finalCol = mix(pureHue * 0.92, pureHue, 0.6);
    fragColor = vec4(finalCol * alpha, alpha);
  } else {
    // Original dark background shader
    col *= 0.45 + 0.7 * e;
