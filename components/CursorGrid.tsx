...
const sampleGradient = (
  ...
  if (palette.length === 0) return [0, 240, 255];
...
export const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const rawColors = (p.colors && p.colors.length > 0)
        ? p.colors
        : (Array.isArray(p.color) ? p.color : [typeof p.color === 'string' ? p.color : '#00F0FF']);
      const palette = rawColors.map(hexToRgb);
      ...
      for (let i = 0; i < alphas.length; i++) {
        ...
        if (isMultiColor) {
          const norm = Math.max(0, Math.min(1, (cx + cy * 0.4) / Math.max(w + h * 0.4, 1)));
          cCore = sampleGradient(palette, Math.max(0, norm * 0.5));
          cMid = sampleGradient(palette, norm);
          cEdge = sampleGradient(palette, Math.min(1, 0.45 + norm * 0.55));
        }

        const gradient = ctx.createRadialGradient(cx, cy, half * 0.1, cx, cy, p.cellSize);
        if (isMultiColor) {
          gradient.addColorStop(0, `rgba(${cCore[0]}, ${cCore[1]}, ${cCore[2]}, ${a})`);
          gradient.addColorStop(0.45, `rgba(${cMid[0]}, ${cMid[1]}, ${cMid[2]}, ${a * 0.9})`);
          gradient.addColorStop(1, `rgba(${cEdge[0]}, ${cEdge[1]}, ${cEdge[2]}, 0)`);
        } else {
        ...
        if (p.fillOpacity > 0) {
          const fillCol = isMultiColor ? cMid : [cr, cg, cb];
          ctx.fillStyle = `rgba(${fillCol[0]}, ${fillCol[1]}, ${fillCol[2]}, ${a * p.fillOpacity})`;
          ctx.fill();
        }
  ...
  useEffect(() => {
    wakeRef.current?.();
  }, [gridOpacity, color, colors, lineWidth, maxOpacity, fillOpacity, cellRadius]);
